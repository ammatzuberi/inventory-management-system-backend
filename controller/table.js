    ...
    if (req.body.name && req.body.quantity && req.body.price && req.body.manufacturer) {
        fs.readFile('./data/userdata.json', (err, data) => {
            if (err) console.log(err)
                ...
                fs.writeFile("./data/userdata.json", JSON.stringify(products), (err) => {
                    if (err)
                        console.log(err);
                    else {
        ...
        fs.readFile('./data/userdata.json', (err, data) => {
            if (err) console.log(err)
                ...
                if (selectedProduct) {
                    ...
                    if (req.body.customername) selectedProduct.name = req.body.customername
                    if (req.body.stock) selectedProduct.stock = req.body.stockIN
                    if (req.body.stock) selectedProduct.stock = req.body.StockOut
